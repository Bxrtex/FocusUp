...
const DASHBOARD_TEMPLATES = [
  {
    ...
    sections: ['hero', 'activeMetric', 'scheduledMetric', 'focusMetric', 'streakMetric', 'coach', 'score', 'roadmap', 'alerts', 'tools', 'snapshot', 'focus', 'upcoming', 'birthdays', 'leaderboard'],
  },
  {
    key: 'work',
    label: 'Work',
    description: 'Priorities, schedule, and blockers.',
    sections: ['hero', 'activeMetric', 'scheduledMetric', 'focusMetric', 'coach', 'score', 'roadmap', 'alerts', 'tools', 'snapshot', 'focus', 'upcoming', 'leaderboard'],
  },
  {
    key: 'sport',
    label: 'Sport',
...
function renderMobileWeekSchedule() {
  ...
  days.innerHTML = Array.from({ length: 7 }, (_, index) => {
    ...
    const tasks = getVisibleTasksForDate(isoDate);
    const taskList = isoDate === selectedDate
...
function getDashboardOverviewData(sectionKey) {
  ...
  const overview = {
    ...
    leaderboard: {
      ...
      copy: 'A quick read on top players and your current pace.',
      stats: [['Players', leaderboard.entries.length], ['Your XP', state.xp], ['Sessions', state.sessions]],
      items: leaderboard.entries.slice(0, 5).map((entry, index) => `#${entry.rank || index + 1} ${getProfileDisplayName(entry)} - ${entry.xp || 0} XP`),
      action: 'Open stats',
      tab: 'stats',
    },
  };
...
document.getElementById('schedule-task-time').addEventListener('input', event => {
  event.currentTarget.closest('.time-field').classList.toggle('has-time', Boolean(event.currentTarget.value));
});
