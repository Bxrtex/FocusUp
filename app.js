...
const OUTLOOK_SCOPES = ['openid', 'profile', 'offline_access', 'User.Read', 'Calendars.ReadWrite'];
const OUTLOOK_AUTH_STORAGE_KEY = 'focusup-outlook-auth';
const OUTLOOK_TOKEN_REFRESH_BUFFER_MS = 2 * 60 * 1000;
const OUTLOOK_SYNC_RANGE_PAST_DAYS = 120;
const OUTLOOK_SYNC_RANGE_FUTURE_DAYS = 365;
const OUTLOOK_SYNC_BODY_MARKER = 'Synced by FocusUp';

// ── State ──
let currentUser = null;
let state = defaultState();
let saveTimeout = null;
let outlookSyncTimeout = null;
let selectedDate = getTodayISO();
let calendarMonth = startOfMonth(selectedDate);
let editingTaskId = null;
let outlookConnection = null;
let outlookSyncInFlight = false;
let suppressOutlookAutoSync = false;
let leaderboard = {
  entries: [],
...
function createTask(task = {}) {
  ...
  return {
    ...
    kind: task.kind || 'task',
    source: task.source || 'focusup',
    outlookEventId: task.outlookEventId || '',
    outlookICalUId: task.outlookICalUId || '',
    outlookLastModified: task.outlookLastModified || '',
...
function isOutlookConfigured() {
  return typeof MICROSOFT_CLIENT_ID === 'string'
    && MICROSOFT_CLIENT_ID
    && !MICROSOFT_CLIENT_ID.startsWith('YOUR_');
}

function getOutlookRedirectUri() {
  return new URL(MICROSOFT_REDIRECT_PATH || 'index.html', window.location.href).toString();
}

function getOutlookStorageKey(suffix) {
  return `focusup-outlook:${currentUser?.id || 'guest'}:${suffix}`;
}

function loadOutlookConnection() {
  if (!currentUser) return null;
  const raw = localStorage.getItem(getOutlookStorageKey('connection'));
  if (!raw) return null;

  try {
    return JSON.parse(raw);
  } catch (error) {
    localStorage.removeItem(getOutlookStorageKey('connection'));
    return null;
  }
}

function saveOutlookConnection(connection) {
  if (!currentUser) return;
  outlookConnection = connection;
  localStorage.setItem(getOutlookStorageKey('connection'), JSON.stringify(connection));
}

function clearOutlookConnection() {
  if (currentUser) {
    localStorage.removeItem(getOutlookStorageKey('connection'));
  }
...
function createRandomString(length = 64) {
  ...
  return Array.from(array, item => item.toString(16).padStart(2, '0')).join('').slice(0, length);
}
...
function formatRelativeSyncTime(isoValue) {
...
function renderOutlookSyncCard(message = '') {
  const badge = document.getElementById('outlook-sync-badge');
  const copy = document.getElementById('outlook-sync-copy');
  const status = document.getElementById('outlook-sync-status');
  const lastSync = document.getElementById('outlook-last-sync');
  ...
  if (!badge || !copy || !status || !lastSync || !connectBtn || !syncBtn || !disconnectBtn) return;

  if (!isOutlookConfigured()) {
    badge.textContent = 'Setup needed';
    badge.className = 'outlook-sync-badge';
    status.textContent = 'Add Microsoft client ID';
    lastSync.textContent = 'Unavailable';
    copy.textContent = 'Set `MICROSOFT_CLIENT_ID` in config.js and add this app as a Single Page Application redirect in Microsoft Entra ID.';
  ...
  const connected = Boolean(outlookConnection?.refreshToken || outlookConnection?.accessToken);
  const syncing = outlookSyncInFlight;
  badge.textContent = syncing ? 'Syncing' : (connected ? 'Connected' : 'Disconnected');
  badge.className = `outlook-sync-badge ${syncing ? 'syncing' : ''} ${connected && !syncing ? 'connected' : ''}`.trim();
  status.textContent = syncing
    ? 'Sync in progress'
    : (connected ? `Ready${outlookConnection?.email ? ` • ${outlookConnection.email}` : ''}` : 'Not connected');
  lastSync.textContent = formatRelativeSyncTime(outlookConnection?.lastSyncAt);
  copy.textContent = message
    || (connected
      ? 'FocusUp will sync scheduled tasks and birthdays with your Outlook calendar. Local changes are pushed automatically after saves.'
      : 'Connect Outlook to import upcoming events and keep FocusUp schedule changes synced back to Microsoft Calendar.');
  connectBtn.disabled = connected || syncing;
  syncBtn.disabled = !connected || syncing;
  disconnectBtn.disabled = !connected || syncing;
}

async function startOutlookConnect() {
  if (!isOutlookConfigured()) {
    showToast('Add Microsoft client ID first');
    renderOutlookSyncCard();
    return;
  }

  const verifier = createRandomString(96);
  const challenge = await createPkceChallenge(verifier);
  const stateValue = createRandomString(32);
  sessionStorage.setItem(OUTLOOK_AUTH_STORAGE_KEY, JSON.stringify({
    state: stateValue,
    verifier,
  ...
  window.location.href = `https://login.microsoftonline.com/${MICROSOFT_TENANT_ID || 'common'}/oauth2/v2.0/authorize?${params.toString()}`;
}

async function exchangeOutlookCodeForTokens(code, verifier) {
  ...
  const response = await fetch(`https://login.microsoftonline.com/${MICROSOFT_TENANT_ID || 'common'}/oauth2/v2.0/token`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error(payload.error_description || payload.error || 'Could not connect to Microsoft');
  }
  return payload;
}

async function refreshOutlookTokens(refreshToken) {
  const body = new URLSearchParams({
    client_id: MICROSOFT_CLIENT_ID,
  ...
  const payload = await response.json();
  if (!response.ok) {
    throw new Error(payload.error_description || payload.error || 'Could not refresh Microsoft token');
  }
  return payload;
}

async function fetchOutlookProfile(accessToken) {
  const response = await fetch('https://graph.microsoft.com/v1.0/me?$select=displayName,userPrincipalName', {
    headers: {
  ...
  const payload = await response.json();
  if (!response.ok) {
    throw new Error(payload.error?.message || 'Could not load Outlook profile');
  }
  return payload;
}

async function finishOutlookAuthIfNeeded() {
  ...
  if (error) {
    renderOutlookSyncCard(decodeURIComponent(errorDescription || error));
    showToast('Outlook connection cancelled');
    return;
  }

  const raw = sessionStorage.getItem(OUTLOOK_AUTH_STORAGE_KEY);
  sessionStorage.removeItem(OUTLOOK_AUTH_STORAGE_KEY);
  if (!raw) {
    renderOutlookSyncCard('The Microsoft login flow expired. Start the connection again.');
    return;
  }

  const authState = JSON.parse(raw);
  if (authState.state !== stateValue) {
    renderOutlookSyncCard('State check failed during Microsoft login.');
    return;
  }

  try {
    const tokenPayload = await exchangeOutlookCodeForTokens(code, authState.verifier);
    ...
    renderOutlookSyncCard('Connected to Outlook. Run the first sync to import existing events.');
    showToast('Outlook connected');
  } catch (authError) {
    clearOutlookConnection();
    renderOutlookSyncCard(authError.message);
    showToast('Outlook connection failed');
  }
}

async function ensureOutlookAccessToken(forceRefresh = false) {
  if (!outlookConnection) {
    outlookConnection = loadOutlookConnection();
  }
  if (!outlookConnection) {
    throw new Error('Connect Outlook first');
  }

  const expiresSoon = !outlookConnection.expiresAt || (Date.now() + OUTLOOK_TOKEN_REFRESH_BUFFER_MS >= outlookConnection.expiresAt);
  if (!forceRefresh && outlookConnection.accessToken && !expiresSoon) {
    return outlookConnection.accessToken;
  }
  if (!outlookConnection.refreshToken) {
    throw new Error('Reconnect Outlook to continue syncing');
  }

  const refreshed = await refreshOutlookTokens(outlookConnection.refreshToken);
  const nextConnection = {
    ...outlookConnection,
    accessToken: refreshed.access_token,
    refreshToken: refreshed.refresh_token || outlookConnection.refreshToken,
    expiresAt: Date.now() + (Number(refreshed.expires_in || 0) * 1000),
  };
  saveOutlookConnection(nextConnection);
...
async function outlookFetch(path, options = {}) {
  ...
  if (response.status === 401) {
    const retryToken = await ensureOutlookAccessToken(true);
    ...
    if (!retry.ok) {
      const errorPayload = await retry.json().catch(() => ({}));
      throw new Error(errorPayload.error?.message || 'Outlook request failed');
    }
  ...
  if (!response.ok) {
    ...
    throw new Error(payload.error?.message || 'Outlook request failed');
  }
...
function getSyncWindow() {
  return {
    start: addDays(getTodayISO(), -OUTLOOK_SYNC_RANGE_PAST_DAYS),
    end: addDays(getTodayISO(), OUTLOOK_SYNC_RANGE_FUTURE_DAYS),
  };
}
...
function mapRepeatToRecurrence(task) {
  if (!task.repeat || task.repeat === 'none') return null;

  const startDate = task.scheduledFor;
  const startDateObj = new Date(`${task.scheduledFor}T12:00:00`);
  ...
  if (task.repeat === 'weekly') {
    return {
      pattern: {
        type: 'weekly',
        interval: 1,
        daysOfWeek: [weekdayMap[startDateObj.getDay()]],
...
function taskToOutlookEvent(task) {
  const isAllDay = !task.scheduledTime || task.kind === 'birthday';
  const baseDate = task.scheduledFor || getTodayISO();
  const displayName = getDisplayName(task);
  const timeZone = getLocalTimeZone();
  const payload = {
    subject: task.kind === 'birthday' ? `${displayName} birthday` : task.name,
    body: {
      contentType: 'text',
      content: `${OUTLOOK_SYNC_BODY_MARKER}\nPriority: ${task.priority}`,
    },
    categories: ['FocusUp'],
  };
  ...
  if (isAllDay) {
    ...
    payload.start = { dateTime: `${baseDate}T00:00:00`, timeZone: 'UTC' };
    payload.end = { dateTime: `${addDays(baseDate, 1)}T00:00:00`, timeZone: 'UTC' };
  } else {
    const endTime = new Date(`${baseDate}T${task.scheduledTime}:00`);
    endTime.setMinutes(endTime.getMinutes() + 30);
    payload.start = { dateTime: `${baseDate}T${task.scheduledTime}:00`, timeZone };
    payload.end = { dateTime: formatLocalDateTime(endTime), timeZone };
  }

  const recurrence = mapRepeatToRecurrence(task.kind === 'birthday'
    ? { ...task, repeat: 'yearly' }
    : task);
  if (recurrence) {
...
function recurrenceToRepeat(event) {
  const type = event.recurrence?.pattern?.type;
  if (!type) return 'none';
  if (type === 'daily') return 'daily';
  if (type === 'weekly' || type === 'relativeWeekly') return 'weekly';
...
function eventToTask(event) {
  ...
  const repeat = recurrenceToRepeat(event);
  const lowerSubject = (event.subject || '').trim().toLowerCase();
  const isBirthday = repeat === 'yearly' && isAllDay && lowerSubject.includes('birthday');

  return createTask({
    name: isBirthday && !lowerSubject.endsWith(' birthday')
      ? `${event.subject} birthday`
      : (event.subject || 'Outlook event'),
    priority: 'medium',
    done: false,
    scheduledFor: startDate,
    scheduledTime: isAllDay ? '' : start.slice(11, 16),
    repeat: isBirthday ? 'yearly' : repeat,
    completedDates: [],
    kind: isBirthday ? 'birthday' : 'task',
    source: 'outlook',
    outlookEventId: event.id,
    outlookICalUId: event.iCalUId || '',
    outlookLastModified: event.lastModifiedDateTime || '',
    lastSyncedAt: syncedAt,
    updatedAt: syncedAt,
  });
}

function updateTaskFromOutlookEvent(task, event) {
  const nextTask = eventToTask(event);
  const syncedAt = nowIso();
  Object.assign(task, {
    name: nextTask.name,
    ...
    kind: nextTask.kind,
    source: 'outlook',
    outlookEventId: nextTask.outlookEventId,
    outlookICalUId: nextTask.outlookICalUId,
    outlookLastModified: nextTask.outlookLastModified,
    lastSyncedAt: syncedAt,
    updatedAt: syncedAt,
  });
}

function findTaskByOutlookEvent(event) {
  return state.tasks.find(task => task.outlookEventId === event.id)
    || state.tasks.find(task => (
      task.outlookICalUId
      && event.iCalUId
      && task.outlookICalUId === event.iCalUId
      && task.scheduledFor === (event.start?.dateTime || '').slice(0, 10)
    ));
}

function hasLocalTaskChangedSinceSync(task) {
  if (!task.lastSyncedAt) return true;
  return new Date(task.updatedAt || 0).getTime() > new Date(task.lastSyncedAt).getTime();
}

function hasRemoteEventChangedSinceSync(task, event) {
  if (!task.outlookLastModified || !event.lastModifiedDateTime) return false;
  return new Date(event.lastModifiedDateTime).getTime() > new Date(task.outlookLastModified).getTime();
}

async function fetchOutlookEventsInWindow() {
  const { start, end } = getSyncWindow();
  const params = new URLSearchParams({
    startDateTime: `${start}T00:00:00Z`,
    endDateTime: `${end}T23:59:59Z`,
    $top: '200',
    $select: 'id,subject,start,end,isAllDay,recurrence,lastModifiedDateTime,iCalUId,categories',
    $orderby: 'start/dateTime',
  });

  const collected = [];
  let nextPath = `/me/calendarView?${params.toString()}`;
  while (nextPath) {
    const payload = await outlookFetch(nextPath);
    collected.push(...(payload.value || []));
    if (payload['@odata.nextLink']) {
      nextPath = payload['@odata.nextLink'].replace('https://graph.microsoft.com/v1.0', '');
    } else {
      nextPath = '';
    }
  }
  return collected.filter(event => event.subject);
}

async function createOutlookEventForTask(task) {
  const created = await outlookFetch('/me/events', {
    method: 'POST',
    body: JSON.stringify(taskToOutlookEvent(task)),
  });

  Object.assign(task, {
    source: task.source || 'focusup',
    outlookEventId: created.id,
    outlookICalUId: created.iCalUId || '',
    outlookLastModified: created.lastModifiedDateTime || '',
    lastSyncedAt: nowIso(),
  });
}

async function deleteOutlookEvent(task) {
  if (!task.outlookEventId) return;
  await outlookFetch(`/me/events/${encodeURIComponent(task.outlookEventId)}`, {
    method: 'DELETE',
  });
}

async function updateOutlookEventForTask(task) {
  const updated = await outlookFetch(`/me/events/${encodeURIComponent(task.outlookEventId)}`, {
    method: 'PATCH',
    body: JSON.stringify(taskToOutlookEvent(task)),
  });
  ...
  if (updated) {
    Object.assign(task, {
      outlookICalUId: updated.iCalUId || task.outlookICalUId || '',
      outlookLastModified: updated.lastModifiedDateTime || task.outlookLastModified || '',
      lastSyncedAt: nowIso(),
    });
  } else {
    task.lastSyncedAt = nowIso();
  }
}

async function syncOutlookCalendar({ silent = false } = {}) {
  if (outlookSyncInFlight) return;
  if (!isOutlookConfigured()) {
    renderOutlookSyncCard();
    return;
  }
  ...
  if (!outlookConnection) {
    renderOutlookSyncCard('Connect Outlook first.');
    if (!silent) showToast('Connect Outlook first');
    return;
  }

  outlookSyncInFlight = true;
  renderOutlookSyncCard();

  try {
    const remoteEvents = await fetchOutlookEventsInWindow();
    const remoteById = new Map(remoteEvents.map(event => [event.id, event]));
    const syncedAt = nowIso();

    suppressOutlookAutoSync = true;

    for (const task of state.tasks.filter(item => !item.scheduledFor && item.outlookEventId)) {
      await deleteOutlookEvent(task);
      task.outlookEventId = '';
      task.outlookICalUId = '';
      task.outlookLastModified = '';
      task.lastSyncedAt = syncedAt;
    }

    for (const task of state.tasks.filter(item => item.scheduledFor)) {
      if (!task.outlookEventId) {
        if (task.source !== 'outlook') {
          await createOutlookEventForTask(task);
          task.lastSyncedAt = syncedAt;
        }
        continue;
      }

      const remoteEvent = remoteById.get(task.outlookEventId);
      if (!remoteEvent) {
        if (task.source === 'outlook' && !hasLocalTaskChangedSinceSync(task)) {
      ...
      remoteById.delete(task.outlookEventId);

      if (hasRemoteEventChangedSinceSync(task, remoteEvent) && !hasLocalTaskChangedSinceSync(task)) {
        updateTaskFromOutlookEvent(task, remoteEvent);
        continue;
      }

      if (hasLocalTaskChangedSinceSync(task) || task.source !== 'outlook') {
        await updateOutlookEventForTask(task);
      }

      task.lastSyncedAt = syncedAt;
    }

    remoteById.forEach(event => {
      const existingTask = findTaskByOutlookEvent(event);
      if (existingTask) {
        updateTaskFromOutlookEvent(existingTask, event);
        return;
      }
      state.tasks.unshift(eventToTask(event));
    });
    ...
    renderAiAddons();
    renderOutlookSyncCard();
    if (!silent) showToast('Outlook synced');
  } catch (syncError) {
    renderOutlookSyncCard(syncError.message);
    if (!silent) showToast('Sync failed');
  } finally {
    suppressOutlookAutoSync = false;
    outlookSyncInFlight = false;
    renderOutlookSyncCard();
  }
}

function scheduleOutlookAutoSync() {
  if (suppressOutlookAutoSync || outlookSyncInFlight) return;
  if (!outlookConnection && currentUser) {
    outlookConnection = loadOutlookConnection();
  }
  if (!outlookConnection) return;

  clearTimeout(outlookSyncTimeout);
  outlookSyncTimeout = setTimeout(() => {
    syncOutlookCalendar({ silent: true });
  }, 1800);
}
...
function disconnectOutlook() {
  ...
  state.tasks = state.tasks.map(task => {
    const nextTask = { ...task };
    nextTask.outlookEventId = '';
    nextTask.outlookICalUId = '';
    nextTask.outlookLastModified = '';
    nextTask.lastSyncedAt = '';
    if (nextTask.source === 'outlook') {
      nextTask.source = 'focusup';
    }
    return nextTask;
  });
  renderTasks();
  ...
  renderSchedule();
  renderBirthdays();
  renderOutlookSyncCard('Outlook has been disconnected on this device.');
  scheduleSave(false);
...
async function initAuth() {
  ...
  outlookConnection = loadOutlookConnection();
  await finishOutlookAuthIfNeeded();
  await loadProfile();
  await loadLeaderboard();
  renderAll();
  if (outlookConnection?.lastSyncAt) {
    const lastSyncAge = Date.now() - new Date(outlookConnection.lastSyncAt).getTime();
...
function scheduleSave(syncOutlook = true) {
  clearTimeout(saveTimeout);
  saveTimeout = setTimeout(saveProfile, 800);
  if (syncOutlook) {
    scheduleOutlookAutoSync();
  }
}
...
function deleteTask(id) {
  ...
  if (taskToDelete.outlookEventId && !outlookSyncInFlight) {
    deleteOutlookEvent(taskToDelete).catch(() => {});
  }
    ...
    <div class="task-item ${isTaskDoneOn(t, today) ? 'done' : ''}">
      ...
      <div class="task-main">
        ...
        <div class="task-meta">
          ...
          ${t.outlookEventId ? '<span class="sync-pill">Outlook</span>' : ''}
        </div>
      </div>
    ...
    <div class="schedule-item ${task.kind === 'birthday' ? 'birthday' : ''} ${isTaskDoneOn(task, selectedDate) ? 'done' : ''}">
      ...
      <div class="schedule-body">
        ...
        <div class="schedule-meta">
          ...
          ${task.outlookEventId ? '<span class="sync-pill">Outlook</span>' : ''}
        </div>
...
document.getElementById('outlook-connect-btn').addEventListener('click', startOutlookConnect);
document.getElementById('outlook-sync-btn').addEventListener('click', () => syncOutlookCalendar());
document.getElementById('outlook-disconnect-btn').addEventListener('click', disconnectOutlook);
