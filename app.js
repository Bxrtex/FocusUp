...
const PREMIUM_STATE_PREFIX = 'focusup-premium-active:';
...
let sharedCalendarChannel = null;
...
function formatTaskSchedule(task) {
  ...
  const base = timeLabel ? `${dateRange} at ${timeLabel}` : dateRange;
...
function formatTaskTime(task) {
  if (!task.scheduledTime) return task.durationMinutes ? formatDuration(task.durationMinutes) : '';
  if (task.scheduledEndTime) return `${task.scheduledTime}–${task.scheduledEndTime}`;
  if (!task.durationMinutes) return task.scheduledTime;
  const [hours, minutes] = task.scheduledTime.split(':').map(Number);
  const totalMinutes = (hours * 60 + minutes + task.durationMinutes) % (24 * 60);
  const endTime = `${String(Math.floor(totalMinutes / 60)).padStart(2, '0')}:${String(totalMinutes % 60).padStart(2, '0')}`;
  return `${task.scheduledTime}–${endTime}`;
}
...
function isPremiumActive() {
  if (isDemoMode || currentUser?.premium) return true;
  return localStorage.getItem(`${PREMIUM_STATE_PREFIX}${currentUser?.id}`) === '1';
}

function setupPremiumOffer() {
  if (isDemoMode || !currentUser || isPremiumActive()) return;
  const createdAt = Date.parse(currentUser.created_at);
  const trialLength = 14 * 24 * 60 * 60 * 1000;
  if (!Number.isFinite(createdAt) || Date.now() - createdAt < trialLength) return;

  const modal = document.getElementById('premium-modal');
  const action = document.getElementById('premium-modal-action');
  if (!modal || !action) return;
  action.href = typeof PREMIUM_PAYMENT_LINK === 'string' ? PREMIUM_PAYMENT_LINK : '#';
...
async function loadSharedCalendars() {
  ...
  const { data, error } = await db.from('shared_calendar_members').select('calendar_id, role, shared_calendars(id, name)').eq('user_id', currentUser.id);
  if (error) return;
  sharedCalendars = (data || []).map(item => ({ id: item.calendar_id, name: item.shared_calendars?.name || 'Shared calendar', role: item.role }));
  activeSharedCalendarId = localStorage.getItem(`focusup-active-calendar:${currentUser.id}`) || null;
  if (activeSharedCalendarId && !sharedCalendars.some(calendar => calendar.id === activeSharedCalendarId)) activeSharedCalendarId = null;
  if (activeSharedCalendarId) await loadSharedCalendarTasks();
}

async function loadSharedCalendarTasks() {
  if (!activeSharedCalendarId) return;
  const { data, error } = await db.from('shared_calendar_tasks').select('*').eq('calendar_id', activeSharedCalendarId).order('updated_at', { ascending: false });
  if (error) return showToast('Could not load shared calendar');
  state.tasks = (data || []).map(task => normalizeTask({
    ...task,
    scheduledFor: task.scheduled_for,
    scheduledUntil: task.scheduled_until,
...
async function selectSharedCalendar(calendarId) {
  if (sharedCalendarChannel) {
    await db.removeChannel(sharedCalendarChannel);
    sharedCalendarChannel = null;
  }
  activeSharedCalendarId = calendarId || null;
  if (currentUser) localStorage.setItem(`focusup-active-calendar:${currentUser.id}`, activeSharedCalendarId || '');
  if (activeSharedCalendarId) {
    await loadSharedCalendarTasks();
    sharedCalendarChannel = db.channel(`shared-calendar-${activeSharedCalendarId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'shared_calendar_tasks', filter: `calendar_id=eq.${activeSharedCalendarId}` }, async () => {
        await loadSharedCalendarTasks();
        renderAll();
      })
      .subscribe();
  ...
  renderAll();
  renderSharedCalendarMembers();
  showToast(activeSharedCalendarId ? 'Shared calendar opened' : 'Personal calendar opened');
}

async function createSharedCalendar() {
  const name = prompt('Shared calendar name', 'Our calendar');
  if (!name?.trim()) return;
  const { data, error } = await db.rpc('create_shared_calendar', { calendar_name: name.trim() });
  if (error) return showToast(error.message);
  await loadSharedCalendars();
  await selectSharedCalendar(data);
}

async function createSharedCalendarFromInput() {
  return createSharedCalendar();
}

async function inviteToSharedCalendar() {
  if (!activeSharedCalendarId) return showToast('Open a shared calendar first');
  const email = prompt('Email address to invite');
  if (!email?.trim()) return;
  const { error } = await db.functions.invoke('invite-calendar', { body: { calendarId: activeSharedCalendarId, email: email.trim() } });
  if (error) return showToast(error.message || 'Could not send invitation');
  showToast('Invitation sent');
}

async function inviteFromInput() {
  const input = document.getElementById('shared-calendar-email');
  if (!input?.value.trim()) return showToast('Enter an email address');
  if (!activeSharedCalendarId) return showToast('Create or select a shared calendar first');
  const { error } = await db.functions.invoke('invite-calendar', { body: { calendarId: activeSharedCalendarId, email: input.value.trim() } });
  if (error) return showToast(error.message || 'Could not send invitation');
  input.value = '';
  showToast('Invitation sent');
}

async function renderSharedCalendarMembers() {
  const list = document.getElementById('shared-calendar-members');
  if (!list || !activeSharedCalendarId) return;
  const { data, error } = await db.from('shared_calendar_members').select('user_id, role').eq('calendar_id', activeSharedCalendarId);
  if (error) return;
  list.innerHTML = (data || []).map(member => `<div class="shared-calendar-member"><span class="shared-calendar-member-avatar">${member.role === 'owner' ? 'O' : 'M'}</span><span>${member.user_id === currentUser?.id ? 'You' : 'Member'}</span><span class="shared-calendar-role">${member.role}</span></div>`).join('') || '<div class="schedule-empty">No members yet.</div>';
}

async function acceptCalendarInvite() {
  const token = new URLSearchParams(window.location.search).get('calendar_invite');
  if (!token || isDemoMode) return;
  const { data, error } = await db.functions.invoke('invite-calendar', { body: { action: 'accept', token } });
  window.history.replaceState({}, '', window.location.pathname);
  if (error || !data?.calendarId) return showToast(error?.message || 'Could not accept invitation');
  await loadSharedCalendars();
  await selectSharedCalendar(data.calendarId);
  showToast('You joined the shared calendar');
}
...
async function saveProfile() {
  ...
  if (activeSharedCalendarId) {
    const rows = state.tasks.map(task => ({
      id: task.id,
      calendar_id: activeSharedCalendarId,
      name: task.name,
      priority: task.priority,
      done: task.done,
      scheduled_for: task.scheduledFor || null,
      scheduled_until: task.scheduledUntil || null,
    ...
    const { data: existingTasks } = await db.from('shared_calendar_tasks').select('id').eq('calendar_id', activeSharedCalendarId);
    const currentIds = new Set(rows.map(row => row.id));
    const deletedIds = (existingTasks || []).map(row => row.id).filter(id => !currentIds.has(id));
    if (deletedIds.length) await db.from('shared_calendar_tasks').delete().eq('calendar_id', activeSharedCalendarId).in('id', deletedIds);
    if (rows.length) await db.from('shared_calendar_tasks').upsert(rows);
    return;
  }
...
function addScheduledTaskFromCalendar() {
  ...
  if (!name) return;
  if (endTime && !timeInput.value) {
    showToast('Set a start time first');
    return;
  }
...
function addTask() {
  ...
  const name = input.value.trim();
  if (!name) return;
  if (scheduleToggle.checked && endTime && !timeInput.value) {
    showToast('Set a start time first');
    return;
  }
  ...
  state.tasks.unshift(createTask({
    ...
    scheduledEndTime: scheduleToggle.checked ? (endTime || '') : '',
    durationMinutes: 0,
...
function saveTaskFromSchedule(id) {
  ...
  if (task.kind === 'birthday') {
    ...
    touchTask(task, {
      ...
      scheduledEndTime: document.getElementById(`${prefix}-end-time`).value,
      durationMinutes: 0,
...
function editTask(id) {
  ...
  const nextEndTime = promptRequiredValue(
    'Edit end time (HH:MM). Leave blank for no end time.',
    task.scheduledEndTime,
    { allowBlank: true, validator: value => /^\d{2}:\d{2}$/.test(value) }
  );
  if (nextEndTime === null) return;
  ...
  touchTask(task, {
    ...
    scheduledEndTime: nextDate ? nextEndTime : '',
    durationMinutes: 0,
...
function parseCustomTimerMinutes(value) {
  ...
  // Check for time range format (e.g., "12.00-18.00", "12:00-18:00", "12-18")
  const timeRangeRegex = /^(\d{1,2})(?:[:.]\d{2})?\s*-\s*(\d{1,2})(?:[:.]\d{2})?$/;
  const timeRangeMatch = raw.match(timeRangeRegex);
  
  if (timeRangeMatch) {
    const startHour = Number.parseInt(timeRangeMatch[1], 10);
    const endHour = Number.parseInt(timeRangeMatch[2], 10);
    ...
    if (startHour >= 0 && startHour <= 23 && endHour >= 0 && endHour <= 23) {
      ...
      if (endHour > startHour) {
        ...
        return null; // Same start and end time
      } else {
  ...
  // Check for simple number (minutes)
  if (/^\d+$/.test(raw)) {
    return Number.parseInt(raw, 10);
  }

  // Check for duration format (e.g., "1h 30min", "90m")
  let totalMinutes = 0;
  let matched = false;
...
function applyCustomTimer() {
  ...
  if (!Number.isInteger(customMinutes) || customMinutes < 1) {
    input.value = '';
    input.placeholder = 'E.g. 90, 30m, 1h 30min, or 12.00-18.00';
    input.focus();
    return;
  }

  input.placeholder = 'E.g. 1h 30min, 90m, or 12.00-18.00';
      ...
      ? `<span class="mobile-week-task-list">
          ${tasks.length
            ? tasks.map(task => `<span class="mobile-week-task">
                ...
                <span class="mobile-week-task-time">${escapeHtml(task.kind === 'birthday' ? 'Birthday' : (formatTaskTime(task) || 'Any time'))}</span>
              </span>`).join('')
...
function renderSchedule() {
  ...
  list.innerHTML = tasks.map(task => {
    ...
    <div class="schedule-item ${task.kind === 'birthday' ? 'birthday' : ''} ${isTaskDoneOn(task, selectedDate) ? 'done' : ''}">
      <div class="schedule-time">${task.kind === 'birthday' ? 'Birthday' : (formatTaskTime(task) || 'Any time')}</div>
      <div class="schedule-body">
        ...
        <div class="schedule-edit-form">
          ...
          <div class="schedule-inline-grid">
            ...
            ${task.kind === 'birthday'
              ? ''
              : `<input class="time-input end-time-input" id="${editPrefix}-end-time" type="time" value="${task.scheduledEndTime || ''}" aria-label="End time" />`}
          </div>
          ${task.kind === 'birthday'
            ? ''
            : `<div class="schedule-inline-grid">
...
function activateTab(tab) {
  if (window.innerWidth <= 640 && tab !== 'shared') tab = 'calendar';
...
document.getElementById('premium-modal-close')?.addEventListener('click', closePremiumOffer);
document.getElementById('premium-modal-later')?.addEventListener('click', closePremiumOffer);
document.getElementById('premium-modal-backdrop')?.addEventListener('click', closePremiumOffer);
...
function renderSharedCalendarControls() {
  const select = document.getElementById('shared-calendar-select');
  const inviteButton = document.getElementById('shared-calendar-invite');
  if (!select) return;
  select.innerHTML = [`<option value="">Personal calendar</option>`, ...sharedCalendars.map(calendar => `<option value="${escapeHtml(calendar.id)}">${escapeHtml(calendar.name)}</option>`)].join('');
  select.value = activeSharedCalendarId || '';
  select.onchange = () => selectSharedCalendar(select.value);
  if (inviteButton) inviteButton.disabled = !activeSharedCalendarId;
  const activeLabel = document.getElementById('shared-calendar-active');
  if (activeLabel) activeLabel.textContent = activeSharedCalendarId
    ? `${sharedCalendars.find(calendar => calendar.id === activeSharedCalendarId)?.name || 'Shared calendar'} is active.`
    : 'Personal calendar is active.';
  renderSharedCalendarMembers();
}
