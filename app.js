...
function durationOptions(selectedMinutes = 0) {
  const options = [
    [0, 'Duration'],
    [15, '15 min'],
    [30, '30 min'],
  ...
  if (Number(selectedMinutes) > 0 && !options.some(([value]) => value === Number(selectedMinutes))) {
    options.splice(1, 0, [Number(selectedMinutes), formatDuration(selectedMinutes)]);
  }
  return options.map(([value, label]) =>
    `<option value="${value || ''}" ${Number(selectedMinutes) === value ? 'selected' : ''}>${label}</option>`
  ).join('');
}
...
function addScheduledTaskFromCalendar() {
  ...
  const durationMinutes = Number(document.getElementById('schedule-duration').value) || 0;
...
function addTask() {
  ...
  const timeInput = document.getElementById('task-time');
  const durationMinutes = Number(document.getElementById('task-duration').value) || 0;
  ...
  state.tasks.unshift(createTask({
    ...
    durationMinutes: scheduleToggle.checked ? durationMinutes : 0,
...
function saveTaskFromSchedule(id) {
  ...
  if (task.kind === 'birthday') {
    ...
    touchTask(task, {
      ...
      durationMinutes: Number(document.getElementById(`${prefix}-duration`).value) || 0,
...
function editTask(id) {
  ...
  const nextDuration = promptRequiredValue(
    'Edit duration in minutes. Leave blank for no duration.',
    task.durationMinutes ? String(task.durationMinutes) : '',
    { allowBlank: true, validator: value => /^\d+$/.test(value) && Number(value) > 0 }
  );
  ...
  touchTask(task, {
    ...
    durationMinutes: nextDate ? (Number(nextDuration) || 0) : 0,
      ...
      <div class="schedule-body">
        ...
        <div class="schedule-edit-form">
          ...
          <div class="schedule-inline-grid">
            ...
            ${task.kind === 'birthday'
              ? ''
              : `<select class="priority-select duration-select" id="${editPrefix}-duration" aria-label="Duration">
                  ${durationOptions(task.durationMinutes)}
                </select>`}
          </div>
          ${task.kind === 'birthday'
            ? ''
